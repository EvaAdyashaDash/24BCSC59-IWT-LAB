...
function checkLoginState() {
  ...
  navLinks.forEach(link => {
    if (link.getAttribute("href") === "login.jsp") {
      if (currentUserStr) {
...
function goNext() {
  let currentUserStr = localStorage.getItem("currentUser");
  if (!currentUserStr) {
    window.location.href = "login.jsp?alert=notlogged";
  } else {
...
window.onload = function () {
  ...
  if (window.location.href.includes("login.jsp") && window.location.search.includes("alert=notlogged")) {
